...
local CloseBtn = CreateTitleBtn("✕", Color3.fromRGB(255, 65, 65), -35)
local MinBtn = CreateTitleBtn("−", Color3.fromRGB(50, 150, 255), -68)

local isMinimized = false
local smoothTween = TweenInfo.new(0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.Out)
MinBtn.MouseButton1Click:Connect(function()
    isMinimized = not isMinimized
    TweenService:Create(MainFrame, smoothTween, {Size = isMinimized and UDim2.new(0, 320, 0, 40) or UDim2.new(0, 320, 0, 420)}):Play()
...
CloseBtn.MouseButton1Click:Connect(function()
    ...
    Settings.Aimbot.HitboxExpander = false
    Settings.Aimbot.WallBang = false
...
-- ========================================== -- 2. نظام التبويبات والتمرير المحسن -- ==========================================
local TabsContainer = Instance.new("Frame")
TabsContainer.Size = UDim2.new(1, -10, 0, 38)
...
local function CreateTabButton(text, targetScroll)
    local btn = Instance.new("TextButton")
    btn.Size = UDim2.new(0, 55, 1, -4)
    btn.BackgroundColor3 = targetScroll.Visible and Color3.fromRGB(0, 170, 255) or Color3.fromRGB(35, 35, 42)
    btn.Text = text
    btn.TextColor3 = Color3.fromRGB(255, 255, 255)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        ComboScroll.Visible = false
        FriendsScroll.Visible = false
        TeleportScroll.Visible = false
        TransmissionScroll.Visible = false
        ...
        for _, otherBtn in ipairs(tabButtons) do
            TweenService:Create(otherBtn, TweenInfo.new(0.2), {
                BackgroundColor3 = (otherBtn == btn) and Color3.fromRGB(0, 170, 255) or Color3.fromRGB(35, 35, 42)
            }):Play()
        end
    end)
end
...
CreateTabButton("Teleport", TeleportScroll)
...
local function CreateToggleItem(parentScroll, text, callback)
    ...
    shadow.Parent = btn
    Instance.new("UICorner", shadow).CornerRadius = UDim.new(0, 7)
...
-- ========================================== -- 4. قائمة Transmission (الأزرار المميزة) -- ==========================================
local function CreateTransmissionButtons(parentScroll)
    ...
    SaveBtn.Text = "💾 حفظ الإحداثيات الحالية"
    ...
    glow.BackgroundColor3 = Color3.fromRGB(0, 200, 150)
    glow.BackgroundTransparency = 0.7
    glow.BorderSizePixel = 0
    glow.Parent = SaveBtn
    Instance.new("UICorner", glow).CornerRadius = UDim.new(0, 8)
    
    local saveGlowTween = TweenService:Create(glow, TweenInfo.new(1, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), {
        BackgroundTransparency = 0.3,
        Size = UDim2.new(1.04, 0, 1.08, 0),
    ...
    TeleportToBtn.Size = UDim2.new(1, 0, 0, 50)
    TeleportToBtn.Position = UDim2.new(0, 0, 0, 58)
    TeleportToBtn.BackgroundColor3 = Color3.fromRGB(255, 100, 50)
    TeleportToBtn.Text = "🚀 انتقال إلى الإحداثيات المحفوظة"
    TeleportToBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
    TeleportToBtn.Font = Enum.Font.GothamBold
    TeleportToBtn.TextSize = 14
    TeleportToBtn.Parent = container
    Instance.new("UICorner", TeleportToBtn).CornerRadius = UDim.new(0, 8)
    
    local glow2 = Instance.new("Frame")
    glow2.Size = UDim2.new(1, 0, 1, 0)
    glow2.Position = UDim2.new(0, 0, 0, 0)
    glow2.BackgroundColor3 = Color3.fromRGB(255, 100, 50)
    glow2.BackgroundTransparency = 0.7
    glow2.BorderSizePixel = 0
    glow2.Parent = TeleportToBtn
    Instance.new("UICorner", glow2).CornerRadius = UDim.new(0, 8)
    
    local teleportGlowTween = TweenService:Create(glow2, TweenInfo.new(1.2, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), {
        BackgroundTransparency = 0.3,
        Size = UDim2.new(1.04, 0, 1.08, 0),
        Position = UDim2.new(-0.02, 0, -0.04, 0)
    })
    teleportGlowTween:Play()
    
    -- مؤشر الحالة
    local statusLabel = Instance.new("TextLabel")
    statusLabel.Size = UDim2.new(1, 0, 0, 22)
    statusLabel.Position = UDim2.new(0, 0, 1, 8)
    statusLabel.BackgroundTransparency = 1
    statusLabel.Text = "⚠️ لم يتم حفظ أي إحداثيات بعد"
    ...
    SaveBtn.MouseButton1Click:Connect(function()
        ...
        if char and char:FindFirstChild("HumanoidRootPart") then
            ...
            SaveBtn.BackgroundColor3 = Color3.fromRGB(50, 255, 50)
            SaveBtn.Text = "✅ تم الحفظ!"
            statusLabel.Text = string.format("✅ تم الحفظ: %.1f, %.1f, %.1f", 
                Settings.Transmission.SavedPosition.X, 
                Settings.Transmission.SavedPosition.Y, 
                Settings.Transmission.SavedPosition.Z)
            ...
            SaveBtn.Text = "❌ لا توجد شخصية"
            statusLabel.Text = "❌ لا توجد شخصية للحفظ"
    ...
    TeleportToBtn.MouseButton1Click:Connect(function()
        if Settings.Transmission.SavedPosition and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
            ...
            local oldColor = TeleportToBtn.BackgroundColor3
            TeleportToBtn.BackgroundColor3 = Color3.fromRGB(0, 150, 255)
            TeleportToBtn.Text = "⏳ جاري الانتقال..."
            
            local targetPos = Settings.Transmission.SavedPosition + Vector3.new(0, 2, 0)
            hrp.CFrame = CFrame.new(targetPos)
            
            -- تأثير بصري
            local particles = Instance.new("Part")
            particles.Size = Vector3.new(3, 3, 3)
            particles.Position = targetPos
            particles.Anchored = true
            particles.CanCollide = false
            particles.Transparency = 0.5
            particles.BrickColor = BrickColor.new("Bright blue")
            particles.Material = Enum.Material.Neon
            particles.Parent = workspace
            
            TweenService:Create(particles, TweenInfo.new(0.5), {Size = Vector3.new(10, 10, 10), Transparency = 1}):Play()
            task.wait(0.5)
            particles:Destroy()
            
            task.wait(0.5)
            TeleportToBtn.BackgroundColor3 = oldColor
            TeleportToBtn.Text = "🚀 انتقال إلى الإحداثيات المحفوظة"
            
            statusLabel.Text = "✅ تم الانتقال بنجاح!"
            statusLabel.TextColor3 = Color3.fromRGB(50, 255, 50)
            
            task.wait(2)
            statusLabel.Text = string.format("📌 المحفوظة: %.1f, %.1f, %.1f", 
                Settings.Transmission.SavedPosition.X, 
                Settings.Transmission.SavedPosition.Y, 
                Settings.Transmission.SavedPosition.Z)
            statusLabel.TextColor3 = Color3.fromRGB(255, 200, 100)
        else
            TeleportToBtn.BackgroundColor3 = Color3.fromRGB(255, 50, 50)
            TeleportToBtn.Text = "❌ لا توجد إحداثيات"
            statusLabel.Text = "❌ لم يتم حفظ أي إحداثيات!"
            statusLabel.TextColor3 = Color3.fromRGB(255, 50, 50)
...
-- ========================================== -- 5. دوال إنشاء قوائم الأصدقاء والانتقال -- ==========================================
local function CreateFriendRow(parentScroll, player)
    ...
    headIcon.Size = UDim2.new(0, 36, 0, 36)
    headIcon.Position = UDim2.new(0, 8, 0.5, -18)
...
local function CreateTeleportRow(parentScroll, player)
    ...
    teleportBtn.Size = UDim2.new(0, 65, 0, 28)
    teleportBtn.Position = UDim2.new(1, -73, 0.5, -14)
...
CreateInputItem(AimbotScroll, "مدى التكبير:", "25", function(val) Settings.Aimbot.HitboxSize = val end)
CreateToggleItem(AimbotScroll, "إطلاق تلقائي (TriggerBot)", function(s) Settings.Aimbot.TriggerBot = s end)
CreateToggleItem(AimbotScroll, "طلقة مخترقة للجدران", function(s) Settings.Aimbot.WallBang = s end)
...
-- ESP Tab
CreateToggleItem(ESPScroll, "تفعيل ESP", function(s) 
...
CreateToggleItem(ESPScroll, "إضاءة الخصوم (Chams)", function(s) Settings.ESP.Chams = s end)

-- Combo Tab
local FloatingTpBtn = Instance.new("TextButton")
FloatingTpBtn.Size = UDim2.new(0, 140, 0, 38)
FloatingTpBtn.Position = UDim2.new(0.5, -70, 0.85, 0)
FloatingTpBtn.BackgroundColor3 = Color3.fromRGB(255, 50, 50)
FloatingTpBtn.Text = "🎯 انتقال للأقرب"
...
Instance.new("UICorner", FloatingTpBtn).CornerRadius = UDim.new(0, 8)
MakeDraggable(FloatingTpBtn)
...
CreateInputItem(ComboScroll, "مضاعف السرعة:", "2", function(val) 
    Settings.Combo.SpeedMultiplier = val
    if Settings.Combo.SpeedBoost and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then 
...
CreateToggleItem(ComboScroll, "تفعيل قوة القفزة", function(s) 
    Settings.Combo.JumpPowerBoost = s
    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then 
        LocalPlayer.Character.Humanoid.JumpPower = s and (50 * Settings.Combo.JumpPowerMultiplier) or 50 
    end 
end)
CreateInputItem(ComboScroll, "مضاعف قوة القفزة:", "2", function(val) 
    Settings.Combo.JumpPowerMultiplier = val
    if Settings.Combo.JumpPowerBoost and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then 
